...
{
	if ($_SESSION['priv1']==6||$_SESSION['priv1']==4)						//checking privileges of user. only Priv 6 (normal user) allowed
	{
		...
		{    
			...
			else
			{
				...
				$query="select * from leave where (employeeid='".$id."' and year='$year') and (nature='".$nature."' and status='approved'";
																//getting leave requests made previously by the same employeewhich were not rejected or cancelled
				...
				$result2=pg_query($conn,"select * from leave where (employeeid='".$id."' and year='$year') and nature='".$nature."'");												//getting leave requests made previously by the same employeewhich were not rejected or cancelled
				...
				else
				{
					...
					$noleftb=$row2[10];								//getting noleft in last leave before the present one
					$nodaysb=$row2[4];								//getting nodays in last leave before the present one
					$noleft=$noleftb-$nodaysb-1;						//getting noleft for the present request
				}
				if($noleft==0)										//no of leaves left = 0
				{
					echo "You have exceeded your total allowed leaves for ".$nature.". Hence, your leave request could not be forwarded.";
				...
				{
					$nolefta=$noleft-$noofdays-1;						//no. of leaves left after applying for this leave
    				$result1=pg_query($conn,"select * from department where name='".$dname."'");
					$row1=pg_fetch_row($result1);					//getting department ID and HOD ID using dept. name
					$did=$row1[0];									//dept id
					$dhod=$row1[3];									//HOD id
